  ...
  <div class="row">
	...
	<p class="text-center">ONLINE SHOPPING WITH BEST DEALS AND BEST PRICES</p>
		...
		$page = $_GET['page'];
			...
			$query_p=mysqli_query($conn, "SELECT * FROM `product` ORDER BY p_id DESC  LIMIT $page1,8");
			do{
		...
		?>
				...
				<h4><b><?php echo $data_p['p_name']; ?></b></h4>
						<h5>Price: <span class="text-danger"><?php echo $data_p['p_price']; ?>$</span></h5>
				<a class="btn btn-danger" type="button" href="products.php?p_id=<?php echo $data_p['p_id']; ?>"><i class="fas fa-cart-plus"></i> Add To Cart</a>
						</div>
					</div>
				</div>
			<?php }} while($data_p); ?>
